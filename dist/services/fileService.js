...
const fileService = {
     ...
     * @returns {string}
     */
    _getFileExtension(filename, mimeType) {
     ...
     */
    async uploadToExternalStorage(fileBuffer, metadata, password = null) {
        try {
            ...
            const minioMetadata = {
                ...
                expiresin: metadata.expires_in.toString(),
            ...
            if (password) {
                minioMetadata.password_hash = await passwordUtils.hashPassword(password);
            }
            metadata.max_downloads =
                parseInt(String(metadata.max_downloads)) || 0;
            metadata.expires_in = parseInt(String(metadata.expires_in)) || 0;
            if (metadata.max_downloads < 0)
                metadata.max_downloads = 0;
            if (metadata.expires_in < 0)
                metadata.expires_in = 0;
            if (password) {
                metadata.password_hash = await passwordUtils.hashPassword(password);
            }
            const ext = this._getFileExtension(metadata.custom_name, metadata.mime_type);
            if (!extname(metadata.custom_name) && ext) {
                metadata.custom_name = metadata.custom_name + ext;
            }
            const key = `${fileId}/${metadata.custom_name}`;
            const result = await minioService.uploadFile(key, fileBuffer, minioMetadata);
            if (!result.success) {
    ...
    async getUploadById(id) {
        try {
            ...
            const metadata = await minioService.getFileMetadata(files.data[0].name);
            if (!metadata.success) {
            ...
            const formattedMetadata = {
                ...
                expires_in: parseInt(metaData.expiresin) || 0,
     ...
     */
    async downloadFromExternalStorage(id, password = null) {
        try {
            const fileInfo = await this.getUploadById(id);
            if (!fileInfo.success || !fileInfo.data) {
                throw new Error("File not found");
            }
            const minioMetadata = await minioService.getFileMetadata(fileInfo.data.key);
            ...
            if (metadata.has_password) {
                ...
                const isValid = await passwordUtils.verifyPassword(storedHash, password);
                if (!isValid) {
                    throw new Error("Invalid password");
                }
            }
            const currentDownloads = parseInt(minioMetadata.data.metaData.downloadcount || "0");
            const maxDownloads = parseInt(minioMetadata.data.metaData.maxdownloads || "0");
            if (maxDownloads > 0 && currentDownloads >= maxDownloads) {
                throw new Error("Maximum download limit reached");
            }
            if (metadata.expires_in > 0) {
                const expirationDate = new Date(new Date(metadata.uploaded_at).getTime() +
                    metadata.expires_in * 60 * 60 * 1000);
            ...
            if (!extname(fileName)) {
                const ext = this._getFileExtension(fileName, metadata.mime_type);
                if (ext) {
            ...
            await minioService.updateMetadata(fileInfo.data.key, updatedMetadata);
     ...
     * @returns {Promise} A promise that resolves with the response object.
     */
    async deleteFromExternalStorage(id) {
        try {
            const fileInfo = await this.getUploadById(id);
            if (!fileInfo.success || !fileInfo.data) {
                throw new Error("File not found");
     ...
     * @returns {Promise} A promise that resolves with the response object.
     */
    async updateUpload(id, metadata, password = null) {
        try {
            const fileInfo = await this.getUploadById(id);
            if (!fileInfo.success || !fileInfo.data) {
                throw new Error("File not found");
            ...
            const result = await minioService.updateMetadata(fileInfo.data.key, updatedMetadata);
            if (!result.success) {
                throw new Error(result.error);
            }
            return {
