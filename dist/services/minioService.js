...
const minioService = {
    ...
    async uploadFile(key, buffer, metadata) {
        try {
            await minioClient.putObject(configuration.storage.minio.bucket, key, buffer, undefined, // Remove metadata from here
            metadata // Pass metadata as last argument
            );
            return { success: true };
        }
        catch (error) {
    ...
    async getFile(key) {
        try {
            const stream = await minioClient.getObject(configuration.storage.minio.bucket, key);
    ...
    async deleteFile(key) {
        try {
            await minioClient.removeObject(configuration.storage.minio.bucket, key);
            return { success: true };
        }
        catch (err) {
    ...
    async listFiles(prefix = "") {
        try {
            const stream = minioClient.listObjects(configuration.storage.minio.bucket, prefix, true);
            const files = [];
    ...
    async getFileMetadata(key) {
        try {
            const stat = await minioClient.statObject(configuration.storage.minio.bucket, key);
    ...
    async updateMetadata(key, metadata) {
        try {
            const stat = await minioClient.statObject(configuration.storage.minio.bucket, key);
            const objectData = await minioClient.getObject(configuration.storage.minio.bucket, key);
            await minioClient.putObject(configuration.storage.minio.bucket, key, objectData, stat.size, metadata);
            return { success: true };
        }
        catch (err) {
