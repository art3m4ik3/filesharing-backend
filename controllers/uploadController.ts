...
const uploadController = {
    ...
    async uploadFile(req: CustomRequest, res: Response) {
        try {
            ...
            const metadata = {
                ...
                expires_at: expiresIn > 0 ? new Date(Date.now() + expiresIn * 60 * 60 * 1000).toISOString() : "",
                uploaded_at: new Date().toISOString(),
                deletion_token: uuidv4(),
                download_count: 0,
            };

            const uploadResult = await fileService.uploadToExternalStorage(
                req.file.buffer,
                metadata,
                password
            );
    ...
    async downloadFile(req: Request, res: Response) {
        try {
            ...
            const fileStream = await fileService.downloadFromExternalStorage(
                id,
                password
            );
            ...
            if (!fileStream.success) {
                ...
                if (fileStream.error === "Maximum download limit reached") {
                    return error(
                        res,
                        403,
                        "Maximum download limit reached for this file"
                    );
                }
            ...
            if (!extname(fileName) && metadata.mime_type) {
                const ext = fileService._getFileExtension(
                    fileName,
                    metadata.mime_type
                );
                if (ext) {
     ...
     * @returns {Promise} A promise that resolves with the response object.
     */
    async updateUpload(req: Request, res: Response) {
        try {
            const { id } = req.params;
            ...
            if (timeDiff > 24) {
                return error(
                    res,
                    403,
                    "Modifications are only allowed within 24 hours of upload"
                );
            }
            ...
            const updatedMetadata = {
                ...
                expires_at: expiresIn && parseInt(expiresIn) > 0
                    ? new Date(Date.now() + parseInt(expiresIn) * 60 * 60 * 1000).toISOString()
                    : metadata.expires_at,
            };

            const updateResult = await fileService.updateUpload(
                id,
                updatedMetadata,
                password
            );
    ...
    async deleteUpload(req: Request, res: Response) {
        try {
            ...
            const uploadTime = new Date(
                fileInfo.data.meta_data.uploaded_at
            ).getTime();
            ...
            if (timeDiff > 24) {
                return error(
                    res,
                    403,
                    "Deletion is only allowed within 24 hours of upload"
                );
            }

            const deleteResult = await fileService.deleteFromExternalStorage(
                id
            );
